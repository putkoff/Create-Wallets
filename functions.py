...
def new_wall():
    ...
    ls = priv,acct.address,''
    return ls
def get_walls():
    js = wall_check()
    print(js)
    return js['adds'],js['priv'],js['names']
def js_it(x):
...
def get_bal(x):
    print(x)
    print(w3.eth.balanceOf(x))
    return w3.eth.functions.getBalance(x)
def send():
...
def is_num(x):
    try:
        ...
        return False
def re_wa():
    if exists('pr_walls.txt') == True:
        return reader_B('pr_walls.txt')
    else:
        return None
def disp_dyn():
    print('checking')
...
def dyn_walls():
    ...
    if re_wa() == None:
        ...
        for i in range(0,len(a)):
            ...
            print(new)
        pen(new.replace('"','').replace(',',''),str('pr_walls.txt'))
        print('printed',new)
